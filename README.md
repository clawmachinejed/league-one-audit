...
- [My Fantasy](docs/my-fantasy.md): compact current-matchup summaries for explicitly selected My Teams, with starting-lineup attention and optional inline detail.
...
- [Estimated win chance](docs/matchup-win-probability.md) on Matchups and My Team, calculated from existing exact-week starter projections, official scores and game progress.
...
With database persistence enabled, the accepted Neon enrollment and season-specific source connections supply the current Sleeper league IDs. Permanent league UUIDs and public route keys survive annual Sleeper renewal; an owner must verify and register the new annual connection. [`apps/site/lib/config.ts`](apps/site/lib/config.ts) retains the explicit bootstrap IDs for database-disabled local development and Preview. Sleeper IDs remain strings because they can exceed JavaScript's safe integer range. The site uses real data, shows empty states when appropriate, and reports unavailable or incomplete data without substituting demonstration teams or results.
...
The [account foundation](docs/account-foundation.md) provides private website accounts and `/my-leagues`, `/account`, and `/sign-in` routes. Registration remains invitation-only with verified email. Accounts are independent of Sleeper identities; saved follows, observed team participation and direct affiliations remain distinct. The League One/Two group leaves Dynasty independent. Authentication and private persistence default off and are disabled in Vercel Preview. [Account league onboarding](docs/account-league-onboarding.md) resolves a Sleeper username, requests profile/team confirmation, and imports supported additional leagues into the existing registry. Discovery alone is read-only. The existing My Team selection contract remains. See the [authentication qualification gates](docs/account-auth-foundation.md) and [database boundary](docs/account-foundation-database.md) before activation.
...
My Fantasy appears immediately before My Team in desktop and mobile navigation; Managers routes and team links remain available. The global `/my-fantasy` page automatically shows signed-in current-season account teams. Guests retain explicit browser My Team choices. Affiliated leagues stay in the ribbon without automatically gaining a personal card. The page reuses Matchups for names, scores, manager avatars, records and places, omitting trophies. Each matchup expands independently; Bench and player statistics open separately. The user stays left; one probability track fills the user's share. Current and projected ranks use proportional thirds. **Enter League ›** opens that league's My Team page. Missing evidence remains unavailable, and each league's failures stay isolated. See [My Fantasy behavior and limits](docs/my-fantasy.md).
...
The global header, browser icon and Apple touch icon use the supplied League One site PNG. League picker and My Fantasy icons use each league's actual Sleeper artwork. Artwork stays square and uncropped. The ribbon labels participating leagues, direct linked leagues and follows; imported leagues use `/leagues/<permanent-key>` routes.

The League page has a **Projected standings** switch above the Rosters tab. It appears only while Standings is selected and is hidden on Waivers, Transactions and Rosters. It starts off on a fresh page load or league change. Turning it on adds the active regular-season week's stored Matchups team projection totals to completed official matchup history and recalculates W–L (including ties), rank, PF, and PA. Ranking uses the existing win percentage, PF, then higher PA tiebreakers. Turning it off restores Sleeper's official standings; Waivers, Transactions, and Rosters always keep their normal data. The projected view never writes results or changes scoring.
...
Rosters shows its week picker in the same upper-right League header area, directly above the Rosters tab. Rosters and Matchups use the same understated week control: a small visible label and dropdown with a larger touch area. The picker marks the current week in its native menu and offers a Current link or button when viewing another week. On the League page it appears only on Rosters; changing tabs preserves the selected week and expanded rosters. Existing exact-week loading, caching, and scheduled refresh behavior remain unchanged.

The site's active scoring and default display weeks advance at noon Eastern local time on the day after the week's last NFL game date, once every scheduled game reports complete. The shared [site week calendar](docs/site-week-calendar.md) controls pages, roster metrics and existing workers independently of Sleeper's display/leg timing. Explicit week selections remain exact, and Current follows rollover in open tabs. Unavailable active-week projections cannot be replaced by an older week's snapshot or estimated starters.
...
Rivalries and a separate statistics section are outside this rebuild's scope.

Managers uses compact cards sized like the League roster summaries. Each card shows the manager above the team name, an avatar and record, and opens the manager profile. Every displayed manager name across the site includes a blue trophy for each verified League One Trophy Bowl win and a red trophy for each verified League Two Promotion Bowl win. Blue trophies use uppercase-letter height, except the original 17×20px size in Managers; red trophies use lowercase-letter height on every screen. Matchup and schedule trophies sit toward the center of the card on both sides. The owner-supplied history follows stable Sleeper manager account IDs across all three leagues, regardless of current team name or relegation; accessible labels and hover text identify each championship league and year. See [championship history and artwork provenance](docs/manager-championships.md). The directory omits the extra section heading, team count, selection banner and PF. My Team can still be selected or cleared on a manager profile; the saved choice remains highlighted across the league.
...
| `ALL_PLAYER_RECURRING_ENABLED` | Optional server-only activation switch. Only exact `true` enables shared hourly all-player collection from noon through midnight Eastern and bounded current-period defensive detail during live games through the existing authenticated live-projection cron; absent is dormant. |
...
`clock-v1` is implemented once in the provider-neutral domain. The application freezes the last eligible pregame baseline at kickoff and calculates each live offensive player or kicker as `official Sleeper points + frozen pregame points × game fraction remaining`. Live D/ST uses earned official points plus the remaining supported frozen defensive components, replacing the provisional points-allowed tier once with its projected final tier. Earned bonuses stay in actuals; future three-and-out, fourth-down-stop and long-touchdown bonuses are not forecast. Exact-period sparse defensive statistics must agree with the official score; missing, stale or contradictory component evidence keeps only that defense on its frozen-baseline fallback. See [live defensive projections](docs/live-defense-projections.md). After an individual NFL game is final, the player's row shows that player's complete immutable frozen baseline while the fantasy-team projected total uses the exact official final result. A missing or invalid frozen final baseline is unavailable in the player row. Team projections remain full-precision sums of the internal phase-appropriate starter contributions and therefore can intentionally differ from the visible player projections.

A whole Tank01 slate must pass plausibility and schedule-aware completeness checks before it can be used. After the slate is trusted and identity matching is safe, an isolated absent or unscorable starter projection uses an explicit zero baseline and remains part of the team sum. A missing frozen baseline after kickoff also uses zero for calculation, is recorded in aggregate observation metadata, and does not cause the worker to invent a frozen database row; after that NFL game is final, the player's visible projection is unavailable rather than substituting that zero, a prior live projection, or the official score. Conflicting or ambiguous identity matches for a rostered player, starter, or required D/ST, duplicate starters, invalid scoring settings, missing active-starter games or clocks, and missing final official points fail closed for the affected league. An unresolved projection-only free agent remains in immutable slate content but is omitted from canonical candidate persistence; shared full-slate coverage records its count, affected position, and an identity-incomplete warning, so no later all-player rank may treat that position as complete. A failed Sleeper league load or later per-league scoring, observation, or publication step is isolated to that league. A shared projection/game-state load, slate-assessment, or provider-persistence failure rejects every league in that group. Partial fleet completion returns the completed counts with failed leagues and makes the cron route return 503; publishing zero leagues fails the run. No failed or incomplete refresh replaces the last complete snapshot.
...
The current cadence observes active periods every minute and future periods every 15, 60 or 360 minutes according to distance. With an 18-week horizon, Week 1 nominal demand is `N × (1 + 1/15 + 3/60 + 13/360)`: approximately 2.31 checks per minute for two leagues, 57.64 for 50 and 345.83 for 300. These counts exclude other provider requests and are offered demand, not measured production throughput. See [measured collection capacity](docs/collection-capacity-validation.md) for the current scheduler replay, isolated Neon timings, storage measurements and their limits.

The shared lineup-check allowance is 20 per minute. Future targets reserve at least one slot, limiting healthy active admission to 19 per invocation; mixed preseason ownership can reduce this to 18. Excess work receives capacity diagnostics and is deferred. Database-backed registration, per-league isolation, guarded claims and publication fences are implemented. Durable continuation, maintenance fairness, provider-rate validation and live-game end-to-end timing remain follow-ups. These measurements establish neither a supported fleet maximum nor a refresh guarantee. Estimated win chance uses the existing snapshots and cadence; see its [model assumptions and release behavior](docs/matchup-win-probability.md).
...
| `docs/collection-isolation.md` | Registration isolation, shared statistics capture, league-specific acceptance, and remaining scale work. |
| `docs/collection-capacity-validation.md` | Measured scheduler limits, isolated Neon capacity, storage, and production observation evidence. |
| `docs/my-fantasy.md` | Cross-league summary scope, saved-selection behavior, data limits and shared refresh paths. |
...
6. Verify production after the merge. Confirm the deployed revision, all configured league identities and route prefixes, core navigation, matchup expansion, independent My Team persistence, and manager transactions as relevant to the change.
