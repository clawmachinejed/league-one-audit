...
| `ALL_PLAYER_RECURRING_ENABLED` | Optional server-only activation switch. Only exact `true` enables shared hourly all-player collection from noon through midnight Eastern after the existing authenticated live-projection cron response; absent is dormant. |
