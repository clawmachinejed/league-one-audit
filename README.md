...
Managers uses compact cards sized like the League roster summaries. Each card shows the manager above the team name, an avatar and record, and opens the manager profile. Every displayed manager name across the site includes one custom trophy for each verified League One Trophy Bowl championship, scaled to the uppercase height of the name font; the Managers directory retains its original 17×20px trophies. Matchup and schedule trophies sit toward the center of the card on both sides. The owner-supplied history follows stable Sleeper manager account IDs across all three leagues, regardless of current team name or relegation; accessible labels and hover text list the championship years. See [championship history and artwork provenance](docs/manager-championships.md). The directory omits the extra section heading, team count, selection banner and PF. My Team can still be selected or cleared on a manager profile; the saved choice remains highlighted across the league.
