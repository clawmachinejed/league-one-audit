...
Vercel invokes the authenticated worker every minute. Current-week work always has priority. The worker exits without contacting Tank01 outside an active sync window, checks hourly before games to keep lineups and pregame projections warm, and runs each minute from two hours before through seven hours after a scheduled kickoff. Only when current work is idle, or the current hourly job already completed, can one durable future action run. League loading and publication concurrency are capped at eight. The projection feed and game-state feed run concurrently for current-week work. A fully cold current group can have three Tank01 requests in flight: weekly projections and the player crosswalk start together, alongside the uncached game-state request. Each Tank01 request has its own bounded timeout. The two projection-side success caches last one hour, so a warm call in which both hit adds no projection HTTP requests. Projection failures use a 60-second process-local backoff, rejected persistent cache loaders are not retained, and sibling projection/crosswalk requests settle before failure is returned.

Future work has an additional whole-operation safety budget. It may not start a new stage after 45 seconds of total worker time and is aborted at 50 seconds. The deadline uses a monotonic clock, races any pending provider promise, and supplies an abort signal to every future-scoped Neon query. A separate four-second bounded cleanup attempts to release the global job before the 60-second Vercel limit; any action lease that cannot be released remains protective until its short expiry. This prevents a slow future refresh from turning into an unbounded database writer after the request has ended.
...
Future Weeks 2–18 use the same public matchup payload and scoring engine through a separate durable preparation path. Sleeper's league state defines the display period and active scoring period; the highest stored snapshot is never treated as the current week. The worker stores one provider-native Tank01 projection slate for each eligible NFL week and shares it across every league. A later materialization action loads fresh Sleeper lineups and one fresh Tank01 game-state slate, scores the stored statistics once per unique raw scoring profile, and publishes a last-known-good snapshot only when the lineup and schedule are complete. It does not call the Tank01 projection endpoint during materialization.

Week 2 is the rollout canary. Later weeks remain closed until every configured league has materialized Week 2 successfully. The worker then permits one future action per cron invocation and staggers each farther week by 15 minutes. Week + 1 projections refresh every six hours and league materializations every hour; Weeks + 2 through + 4 refresh daily; farther weeks refresh every seven days. Failures retry after 5 minutes, 15 minutes, 1 hour, and then 6 hours. Future freshness comes from the durable provider-slate and Sleeper-source lineage plus the recorded next refresh date, rather than the age of the snapshot itself. Details and operational limits are recorded in [the future-week projection guide](docs/future-week-projections.md).
...
Deterministic synthetic tests exercise the current and future paths independently. Current-week tests cover 3, 50, and 300 leagues and retain their original lightweight one-starter fixture. Future-path tests cover 2, 3, 50, and 300 twelve-manager leagues with six matchups per league and all 16 NFL games. They prove one shared Tank01 projection-feed invocation per period, zero projection-feed calls during materialization, one shared game-state invocation, one stored-slate read, concurrency capped at eight, bounded outstanding work, and stable selections, revisions, payloads, and hashes across repeated fixed inputs.

The two-league MVP fits the current single Vercel worker. The synthetic evidence does not prove that 50 or 300 leagues fit Vercel's 60-second function limit. At 300 leagues, publication alone requires roughly 38 waves at concurrency eight, with several sequential database stages in each wave. Before that scale, the system still needs partitioned durable jobs, renewable and publication-fenced leases, provider-rate testing, backpressure, chunked identity work, and real provider/Neon load testing. The hard future-work deadline is a safety boundary, not a capacity claim. Real 2026 in-game status, clock-transition, halftime, overtime, and final convergence checks are also deferred until games are actually in progress. Live matchup win probability is not implemented.
...
The projection engine has one scheduled job at `/api/cron/live-projections`, declared in `apps/site/vercel.json`. It is authenticated with `CRON_SECRET`. Current and future global jobs use the existing 120-second database lease to suppress overlap, while individual future actions use a 55-second durable lease. Future work stops admitting stages after 45 seconds and aborts at 50 seconds. Leases are not renewed and do not fence snapshot publication by lease owner; renewable, fenced, partitioned work remains required before large-scale operation. This cron is the only browser-independent refresh path. The existing Git integration is connected; still inspect the actual preview and production deployment for each release rather than assuming a push succeeded.
