...
During an active season, Current and the initial Matchups/Rosters selection follow Sleeper's validated active scoring week, even when its display week leads or lags. Explicit week selections remain exact. Preseason, completed seasons, and unavailable active authority retain the bounded display fallback. Provider display metadata and stored API defaults remain unchanged; the Matchups page requests the exact active week when those defaults differ. Unavailable active-week projections cannot be replaced by an older week's snapshot.
