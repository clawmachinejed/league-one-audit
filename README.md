...
The [account foundation](docs/account-foundation.md) adds private website accounts and `/my-leagues`, `/account`, and `/sign-in` routes. Accounts are independent of Sleeper identities; saved follows, observed team participation and direct league affiliations remain distinct. The optional League One/Two group leaves Dynasty independent. Authentication and private persistence default off and are disabled in Vercel Preview. My Leagues includes current-season Sleeper links and an [automatic settings capability report](docs/league-capabilities.md); discovery does not enroll additional leagues. The existing My Team pages and public collection behavior are unchanged; an account-wide teams contract is available for their later redesign. See the [authentication qualification gates](docs/account-auth-foundation.md) and [database boundary](docs/account-foundation-database.md) before activation.
...
My Fantasy appears immediately before My Team in desktop and mobile navigation; Managers is no longer in the ribbon, while its routes and team links remain available. The global `/my-fantasy` page currently shows cards only for explicit My Team choices saved on each league's manager profile, including in production. Affiliated leagues remain in the league selector without automatically gaining a card. This temporary browser selection rule will later be replaced by the signed-in account's provider teams. The page reuses Matchups for team names, scores, manager usernames/avatars, records and places, omitting trophies. Each matchup expands independently; Bench and paired player statistics open separately. The user stays left; one full-width probability track fills only the user's share. Current and projected ranks use proportional thirds, not playoff/relegation colors. The separate **Enter League ›** link stays on header row two and opens that league's My Team page. Missing evidence remains unavailable, and each selected league's failures stay isolated. See [My Fantasy behavior and limits](docs/my-fantasy.md).
