...
/** EXACTLY LIKE STANDINGS: prefer full/relative in metadata.avatar; else user.avatar hash → thumbs */
function normalizeSleeperImage(v?: string): string | undefined {
  if (!v) return;
  const s = v.trim();
  if (!s) return;
  if (/^https?:\/\//i.test(s)) return s;                    // full URL
  if (s.startsWith("/")) return `https://sleepercdn.com${s}`; // relative path
  return `https://sleepercdn.com/avatars/thumbs/${s}`;       // avatar_id hash
}
...
function groupByMatchup(list: Matchup[] | null | undefined): Map<number, Matchup[]> {
...
export default async function MatchupsWeekPage(props: { params: Promise<{ week: string }> }) {
  ...
  if (!lid) {
    return (
      <main className="page">
        <h1>Matchups</h1>
        <p>
          Missing league id. Set <code>SLEEPER_LEAGUE_ID</code> or <code>NEXT_PUBLIC_SLEEPER_LEAGUE_ID</code>.
        </p>
      </main>
    );
  }
  ...
  const week = Math.max(1, asNum(weekParam, currentWeek)); // default to current week if param invalid

  // users + rosters → names & avatar urls (same approach as Standings)
  ...
  const list = await j<Matchup[]>(`/league/${lid}/matchups/${week}`, 15);
  const groups = Array.from(groupByMatchup(list))
    .map(([mid, arr]) => ({ id: mid, a: arr[0], b: arr[1] }))
    .filter((x) => x.a && x.b);

  const isCurrent = week === currentWeek;
  const final = false; // style: live view never colors as win/loss; keep neutral during live

  return (
    <main className="page" style={{ display: "grid", gap: 12 }}>
      <h1 style={{ marginBottom: 4 }}>Matchups — Week {week}</h1>
      <p style={{ color: "#6b7280", marginTop: -6 }}>
        {isCurrent ? "Auto-refreshing scores (every ~25s)." : "Final / historical week."}
      </p>
      ...
      <div className="m-grid">
        {groups.length === 0 ? (
          ...
          groups.map((g) => {
            ...
            const aAvatar = avatarByRosterId.get(aRid) || "/avatar-placeholder.png";
            const bAvatar = avatarByRosterId.get(bRid) || "/avatar-placeholder.png";
              ...
              <div className="m-card" key={g.id}>
                <div className="m-row">
                  ...
                  <div className="m-score">
                    <span className={aWin ? "won" : final && bWin ? "lost" : ""}>{aPts.toFixed(2)}</span>
                  </div>
                </div>
                ...
                <div className="m-row">
                  ...
                  <div className="m-score">
                    <span className={bWin ? "won" : final && aWin ? "lost" : ""}>{bPts.toFixed(2)}</span>
                  </div>
                </div>

                <div className="m-meta">
                  <span className={isCurrent ? "live" : "muted"}>{isCurrent ? "Live" : "Final"}</span>
                </div>
              </div>
            );
          })
      ...
      {/* live polling only when viewing current week */}
      {isCurrent && (
        <script
