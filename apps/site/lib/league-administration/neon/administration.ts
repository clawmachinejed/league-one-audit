...
export function createLeagueAdministrationMethods(client: DatabaseClient): Omit<LeagueAdministrationStore, 'enabled'> {
  ...
  return {
    ...
    listEnrollmentInventory: season => readEnrollmentInventory(client, season),
    async readEnrollment(selector, season) {
      const { entries } = await readEnrollmentInventory(client, season, selector);
      if (entries.length > 1) throw new Error('Ambiguous administration enrollment.');
      return entries[0] ?? { status: 'missing' };
    },
    async listEnrollments(season) {
      const { entries } = await readEnrollmentInventory(client, season);
      if (!entries.length) throw new Error('Enabled league administration has no active enrollments.');
      return entries.map(entry => {
        if (entry.status !== 'ready') throw new Error(`Missing administration registration: ${entry.reason}.`);
