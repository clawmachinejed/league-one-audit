  ...
  return {
    async recordGameStates(input) {
      ...
      const rows = await client.query(`/* projection-store:record-game-states */
        SELECT external_game_id, source_revision, observation_id
        FROM public.record_game_state_observations($1::text, $2::jsonb)
        ORDER BY external_game_id`, [normalizedProvider, json(states)]);
      return {
        kind: 'stored',
        value: rows.map((row) => ({
