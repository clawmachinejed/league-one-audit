...
import { createLiveDefenseReadMethods } from './projections/adapters/neon/live-defense-reader';
...
export function createProjectionStore(database: Database = getDatabase()): ProjectionStore {
  ...
  const liveDefenseReads = createLiveDefenseReadMethods(client);
  ...
  return {
    ...
    markAllPlayerRequest: jobs.markAllPlayerRequest,
    finishAllPlayerJob: jobs.finishAllPlayerJob,
    finishLiveDefenseStatRequest: jobs.finishLiveDefenseStatRequest,
    recordAllPlayerPreclaimOutcome: jobs.recordAllPlayerPreclaimOutcome,
