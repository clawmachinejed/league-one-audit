  ...
  return <>
    ...
    {data.teams.length ? <div className="standings-wrap"><table className="standings-table">
      ...
      <tbody>{data.teams.map((team, index) => <tr key={team.id} className={selected === team.id ? 'selected-row' : ''}>
        ...
        <th scope="row"><Link href={`${site.prefix}/owners/${team.id}`} className="standings-team"><Avatar team={team} /><span className="team-text"><span className="team-name">{team.name}</span><span className="owner-name">{selected === team.id && <span className="my-team-label">MY TEAM<span aria-hidden="true"> · </span></span>}{team.ownerName}</span></span></Link></th>
