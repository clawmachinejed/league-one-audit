...
function AssetCard({ activity }: { activity: LeagueMoveActivity | LeagueTradeActivity }) {
  const outcome = resultClass(activity.result);
  return <article className={`transaction-card league-activity-card result-${outcome}`} data-kind={activity.kind}>
    <div className="transaction-header"><div><p className="transaction-type">{activity.title}</p><p className="transaction-subtype">{activity.kind === 'trade' ? 'Trade' : activity.type}</p><p className="transaction-date">{transactionDateLabel(activity.timestamp)}</p></div><span className={`result-badge ${outcome}`}>{activity.result}</span></div>
...
function playerDetails(activity: LeagueWaiverActivity) {
  if (!activity.player) return null;
  return [activity.player.position === '—' ? null : activity.player.position, activity.player.nflTeam].filter(Boolean).join(' · ');
}
...
function WaiverCard({ activity }: { activity: LeagueWaiverActivity }) {
  ...
  const details = playerDetails(activity);
  ...
  return <article className={`transaction-card league-activity-card waiver-card result-${hasWinner ? 'positive' : 'negative'}`} data-kind="waiver">
    <div className="transaction-header waiver-card-header"><div><div className="transaction-title-row"><p className="transaction-type">{activity.player?.name ?? 'Unknown player'}</p><span>Waiver</span></div>{details && <p className="transaction-player-details">{details}</p>}<p className="transaction-date">{transactionDateLabel(activity.processedAt)} · {countLabel}</p></div>{hasWinner && <span className="result-badge positive">Won</span>}</div>
    <div className="transaction-body waiver-card-body">
      {hasWinner ? <dl className="transaction-lines waiver-winning-moves">{activity.winners.flatMap(winner => {
        const rows = [];
        if (winner.added.length) rows.push(<div key={`${winner.id}-added`} className="movement-add"><dt>Added</dt><dd>{winner.added.map(player => player.name).join(', ')} to {winner.team}</dd></div>);
        if (winner.dropped.length) rows.push(<div key={`${winner.id}-dropped`} className="movement-drop"><dt>Dropped</dt><dd>{winner.dropped.map(player => player.name).join(', ')} from {winner.team}</dd></div>);
        return rows;
    ...
    {state === 'ready' && data && <>
      ...
      <div id={`${id}-activity-list`} role="tabpanel" aria-labelledby={`${id}-${filter}-filter`}>
        {filtered.length ? <div className="transactions-list league-transactions-list">{filtered.map((activity: LeagueTransactionActivity) => activity.kind === 'waiver'
          ...
          : <AssetCard key={activity.id} activity={activity} />)}</div>
