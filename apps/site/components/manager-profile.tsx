...
import type { ManagerData, TransactionsData } from '../lib/types';
...
export function ManagerHeader({ data, active }: { data: ManagerData | TransactionsData; active: 'roster' | 'transactions' }) {
  ...
  return <>
    ...
    <nav className="profile-tabs" aria-label="Team pages"><Link href={`${site.prefix}/managers/${data.team.id}`} aria-current={active === 'roster' ? 'page' : undefined}>Roster</Link><Link href={`${site.prefix}/managers/${data.team.id}/transactions`} aria-current={active === 'transactions' ? 'page' : undefined}>Transactions</Link></nav>
  </>;
}
