...
import { ordinal, RosterContent } from './rosters-view';
...
const data: RostersData = {
  ...
  teams: [{
    ...
    sections: [{ name: 'Starters', players: [{
      ...
      slot: 'QB', positionRank: 8, ppg: 19.4, byeWeek: 12,
    }] }, { name: 'Bench', players: [] }],
  }, {
...
describe('rosters presentation', () => {
  ...
  it('shows shared headings once and values without repeated metric labels', () => {
    ...
    expect(html.match(/>POS</gu)).toHaveLength(2);
    expect(html.match(/>PPG</gu)).toHaveLength(2);
    expect(html.match(/>BYE</gu)).toHaveLength(2);
    expect(html).toContain('>QB8<');
    expect(html).toContain('>19.4<');
    expect(html).toContain('>12<');
    expect(html).toContain('Sun 1:00 PM @ HOU');
    expect(html).not.toMatch(/actual|projected|time remaining|live game clock/iu);
  });

  it('puts My Team first, retains its real rank, expands it by default, and keeps content outside the toggle', () => {
    const html = renderToStaticMarkup(<RosterContent data={data} selected={2} />);
    expect(html.indexOf('A Team Name')).toBeLessThan(html.indexOf('First Place'));
    expect(html).toContain('record 6–2, standings 2nd');
    expect(html).toMatch(/<button[^>]*data-roster-toggle="true"[^>]*aria-expanded="true"[^>]*>.*?<\/button><div[^>]*>.*?An Exceptionally Long Quarterback Name/su);
  });

  it('uses compact overflow-safe geometry and a bottom-right-only chevron', () => {
