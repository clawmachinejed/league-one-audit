...
export function rosterCacheKey(leagueKey: string, week: number): string {
  return `${leagueKey}:${week}`;
}
...
export function RostersView({ active, league, selected }: { active: boolean; league: League; selected: number | null }) {
  ...
  const cache = useRef(new Map<string, RostersData>());
  const requestGeneration = useRef(0);

  useEffect(() => {
    if (!active) return;
    const key = rosterCacheKey(site.key, week);
