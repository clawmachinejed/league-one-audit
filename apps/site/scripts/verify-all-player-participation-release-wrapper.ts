...
import { buildAllPlayerParticipationReleaseWrapper, releaseWrapperSha256,
...
const migrationName = '012_all_player_provider_participation.sql';
...
const previousManifest = JSON.parse(await readFile(new URL(
  '../release/011-catalog.integration.json', import.meta.url), 'utf8')) as AllPlayerRepairManifest;
...
try {
  ...
  if (Number(version[0].version) < 180000 || Number(version[0].version) >= 190000) {
    throw new Error('Migration 012 wrapper validation requires PostgreSQL 18.');
  }
  ...
  await writeFile(new URL('../release/012-catalog.integration.json', import.meta.url),
  ...
  await prepareIntegrationDatabase({ throughMigration: '011_all_player_foundation_guards.sql' });
  ...
  let rejected = false;
  try { await execute(buildAllPlayerParticipationReleaseWrapper(corrupt)); } catch { rejected = true; }
  ...
  if (!rejected || JSON.stringify(before) !== JSON.stringify(rolledBack)
    ...
    throw new Error('The corrupted 012 constraint manifest did not fully roll back the release.');
  }
  const result = await execute(buildAllPlayerParticipationReleaseWrapper(input));
  const sentinel = `ALL_PLAYER_PARTICIPATION_APPLIED:${migrationName}:${migrationChecksum}`;
  if (!result.some((row) => row.success_sentinel === sentinel)) {
    throw new Error('Migration 012 success sentinel is missing; do not retry an ambiguous release.');
  }
