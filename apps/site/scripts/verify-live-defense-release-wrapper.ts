...
import { LIVE_DEFENSE_MIGRATIONS, LIVE_DEFENSE_INSTALLED_LEDGER, liveDefenseCatalogSql, liveDefenseDefinitionsSql,
  buildLiveDefenseReleaseWrapper, requireLiveDefenseReleaseSentinel,
...
const catalog = async (affected = true): Promise<AdministrationCatalog> => (
  await ownerQuery<{ catalog: AdministrationCatalog }>(liveDefenseCatalogSql({ affected }))
)[0].catalog;
...
let prepared = false;
try {
  await prepareIntegrationDatabase({ throughMigration: LIVE_DEFENSE_INSTALLED_LEDGER.at(-1)![0] });
  prepared = true;
  const version = await ownerQuery<{ version: string }>("SELECT current_setting('server_version_num') AS version");
  if (Number(version[0].version) !== ADMINISTRATION_POSTGRES_VERSION) throw new Error('Live-defense release capture requires PostgreSQL 180006.');
  const before = await catalog();
  const beforeDefinitions = (await ownerQuery<{ definitions: Record<string, unknown> }>(liveDefenseDefinitionsSql()))[0].definitions;
  const unaffectedBefore = await catalog(false);
  const beforeLedger = await ledger();
  await prepareIntegrationDatabase({ throughMigration: LIVE_DEFENSE_MIGRATIONS[0] });
  const after = await catalog();
  if (JSON.stringify(await catalog(false)) !== JSON.stringify(unaffectedBefore)) {
    throw new Error('Live-defense migration changed an unexpected existing catalog object or ACL.');
  }
  ...
  await writeFile(new URL('../release/live-defense/catalog.integration.json', import.meta.url), `${JSON.stringify(manifest, null, 2)}\n`);
  const definitions = (await ownerQuery<{ definitions: Record<string, unknown> }>(liveDefenseDefinitionsSql()))[0].definitions;
  await writeFile(new URL('../release/live-defense/catalog-definitions.integration.json', import.meta.url),
  ...
  let refused = false;
  try { await execute(buildLiveDefenseReleaseWrapper(corrupted)); } catch { refused = true; }
  if (!refused || JSON.stringify(await catalog()) !== JSON.stringify(before)
    ...
    throw new Error('A corrupt live-defense function capture failed to roll back catalog and ledger atomically.');
  }
  ...
  try { await execute(buildLiveDefenseReleaseWrapper(corruptConstraints)); } catch { constraintsRefused = true; }
  ...
  const rows = await executeWithContinuingReplayChecks(buildLiveDefenseReleaseWrapper(input),
    buildLiveDefenseReleaseWrapper({ ...input, expectedDatabase: `${env.expectedDatabase}_wrong` }));
  const sentinel = requireLiveDefenseReleaseSentinel(rows, migrations);
  if (JSON.stringify(await catalog()) !== JSON.stringify(after)
    || JSON.stringify(await catalog(false)) !== JSON.stringify(unaffectedBefore)) {
    throw new Error('The actual live-defense release wrapper produced a different catalog.');
  }
  let replayRefused = false;
  try { await execute(buildLiveDefenseReleaseWrapper(input)); } catch { replayRefused = true; }
  if (!replayRefused || JSON.stringify(await catalog()) !== JSON.stringify(after)) {
    throw new Error('An already installed live-defense migration was not refused without changes.');
  }
  const evidence = { outcome: 'passed', postgresVersion: ADMINISTRATION_POSTGRES_VERSION,
    ...
    checks: ['exact-001-through-018-ledger', 'postgres-180006-not-null-constraint-catalog',
  ...
  await writeFile(new URL('../release/live-defense/wrapper-verification.integration.json', import.meta.url), `${JSON.stringify(evidence, null, 2)}\n`);
