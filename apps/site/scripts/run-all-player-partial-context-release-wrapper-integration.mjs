...
if (process.argv.length !== 2) throw new Error('Partial-context wrapper integration accepts no arguments.');
if (process.env.PROJECTION_INTEGRATION_AUTHORIZATION !== 'I_ACKNOWLEDGE_THIS_RESETS_AN_ISOLATED_DATABASE') {
  throw new Error('Partial-context wrapper integration requires the exact destructive-test authorization.');
}
for (const name of ['PROJECTION_INTEGRATION_OWNER_DATABASE_URL', 'PROJECTION_INTEGRATION_RUNTIME_DATABASE_URL',
  ...
  if (!process.env[name]?.trim()) throw new Error(`Partial-context wrapper integration requires ${name}.`);
}
process.env.PROJECTION_INTEGRATION_ENV_FILE = '.env.integration.local';
await import('./verify-all-player-partial-context-release-wrapper.ts');
