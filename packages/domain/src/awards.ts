// packages/domain/src/awards.ts
import type { Matchup, BenchStat } from "@l1/contracts";

export type WeeklyAwards = {
  high_score: number;
  narrowest_win: number;
  bench_of_week: number;
};
...
export function computeWeeklyAwards(
  ...
  const benchByMatchup: Record<number, BenchStat> = {};
