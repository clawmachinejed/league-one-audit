...
After an authorized production release, verify the exact merged Git SHA in the Ready Vercel deployment; source repo, main branch, and apps/site root; both leagues; relevant public revision/full endpoints; naturally scheduled worker results; leases; A3A-observed allowance/reset plus current-live and discretionary-future burn, and, when A3B was released, its supported reserve and admitted envelope; and the approved rollback owner. Do not force idle production work for evidence.
...
1. Provider cost: after A1 refreshes the account identity, pricing, quota, safely available reset/weight facts, and current scheduled demand, the user may keep capacity already proved adequate or approve an exact A2 plan change. A2 never depends on A3A: it may proceed from A1 and exact user approval as a precautionary operation, while any A3A evidence that already exists is optional input. A2 must label its outcome either “proved sustainable capacity change” when the full allowance/reset/weight/demand/retry/margin envelope is defensible, or “precautionary risk-reduction upgrade” while a necessary fact remains Missing proof. A precautionary upgrade may unlock observation-only A3A but cannot by itself unlock A3B, be called sustainable, or close OPS-001. After sufficient A3A evidence, A3B may proceed through either a proved full-demand capacity path or a proved admission-feasible path in which unavoidable current-live work, its cold-isolate/retry envelope, and incident margin fit, while bounded discretionary future work can be durably deferred so the admitted envelope fits. If that unavoidable current-live envelope plus margin does not fit, A3B remains blocked pending a proved sustainable A2 change or a separately approved workload/cadence redesign. If existing capacity is not proved safe for observation and the user does not approve the precautionary operation, A3A stops on that explicit safety decision; it is not waiting for proof from itself. No subscription may change without approval of the exact price and terms. A key rebind or workload/cadence redesign outside A3B's bounded future-work admission is materially different work and requires its own approved plan.
...
| 6 | B2 Preview persistence intentionally disabled | PREVIEW-001 | Completed B1 target-identity/catalog evidence | Disable Preview at the existing database boundary |
...
| 11 | D2 Future cache/freshness alignment | FUTURE-001 | A3A; A3B for fresh ingest, any increase, reset-period shift, or unproved delta | Zero-delta retiming or A3B-bounded fresh ingest |
...
| 30 | J1C Production environment cleanup | ENV-001 | J1B; separate Production removal, post-removal redeploy, and any rotation authority | Retirement/disposition plus new Ready-deployment proof |
...
- Closure owner: Satisfies the capacity-change portion of OPS-001 only for a proved sustainable capacity change. A precautionary risk-reduction upgrade may allow A3A observation but leaves OPS-001 open. Later A3A evidence may independently prove either the full scheduled envelope or only A3B's narrower current-live-plus-retry-and-margin/post-deferral entry gate. The narrower result may permit A3B but does not retroactively relabel A2 as sustainable; neither result closes OPS-001 without demonstrated A3B admission.
- Individual prompt:

> In a fresh task, execute only A2 from docs/audits/2026-09-04-production-systems-audit-remediation.md after A1 and exact user cost approval; never wait for A3A, though any already available A3A evidence may be used. Present the exact current price and terms, and change only the user-approved Tank01 subscription on the attested account. Label the result “proved sustainable capacity change” only when the full reset/weight/demand/retry/margin envelope is defensible; otherwise label it “precautionary risk-reduction upgrade,” leave OPS-001 open, and use it only to permit normal-call A3A observation. The precautionary label never unlocks A3B; any later A3B entry must rest on sufficient A3A proof of its full-capacity or current-live-plus-retry-and-margin/post-deferral gate. Do not edit code, keys, Vercel, cron, scoring, or data, and do not make a diagnostic provider call. Stop before financial action unless the user approved that exact plan and price.
...
- Findings: OPS-001 — Operational risk / Critical / High — Tank01 hard cap cannot support scheduled production demand.
- Evidence/reproduction: Use A1, any applicable A2 state, and representative normal-call A3A evidence. Before editing, document an evidence-sufficiency gate covering supported allowance per reset period; billed units or a defensible upper bound per used endpoint class; natural game-day burn; unavoidable current-live demand including its cold-isolate/retry envelope; discretionary future demand; and explicit incident margin. Prove one path: either the full scheduled envelope fits, or current-live demand plus retries and margin fits while a reviewed current-before-future policy can keep the post-deferral envelope within the same allowance by durably deferring discretionary future ingestion. The pre-admission total may exceed the allowance. If a fact needed to bound the selected path is unknown, current-live demand plus retries and margin does not fit, or no truthful post-deferral envelope fits, retain the gap as Missing proof and do not start A3B.
- Dependencies/prerequisites: A3A released and stable with enough representative evidence; either the full scheduled envelope fits, or current-live demand plus retries and margin fits and the proposed policy is proved capable of keeping the post-deferral envelope within the supported allowance/reset period; G0, G1, G3, G5; explicit approval of the current-live/discretionary classification, derived reserve, maximum deferral/backlog freshness policy, alert and recovery owners, admission behavior, and production release.
- Protected behavior: Unavoidable current-live work, including its retry/cold-isolate envelope, is never deferred, dropped, or silently degraded; only evidence-classified discretionary future work may wait. Browser/observer Tank01 calls remain zero; two leagues sharing one provider period still share requests; exact-week, call grouping, cache, last-known-good, scoring, and existing cron definitions remain unchanged.
- In scope: Derive and enforce a supported quota reserve and deterministic admitted-envelope calculation from measured/proved inputs; alert on evidence disagreement and low headroom; defer only approved discretionary future ingestion before current live work; bound backlog age and post-reset catch-up; make every defer/resume durable and truthful without false success; add no provider calls or traffic.
- Excluded: Deferring or reducing unavoidable current-live accuracy; treating current work as discretionary to make the calculation fit; unbounded starvation/catch-up; observation-field expansion unrelated to the reserve; provider plan/key change; cron/cadence expansion; scoring/database/API change; guessed reset/weight; raw provider data; and any claim that an unknown fact is proven.
- Tests/evidence: Measured custom billing weights and reset transition; absent/malformed/conflicting telemetry remains Missing proof and fails closed without inventing a threshold; full pre-admission demand over capacity with current-live-plus-retries-and-margin inside capacity yields a bounded truthful post-deferral envelope; current-live-plus-retries-and-margin equal to capacity preserves current and admits no discretionary work; current-live-plus-retries-and-margin over capacity blocks A3B; no current work is denied because of future work; deferred future work resumes after reset/headroom without starvation, burst, or false success; 429/hard cap and 5xx; cold/warm cache; retries and multiple isolates; shared multi-period grouping; no provider calls are added and period counts do not exceed the admitted envelope; both-league isolation.
- Release checks: G0, G1, G3, G4, G5; exact preview and production SHA; A3A natural-call reconciliation; proved full-demand fit or proved current-live-plus-retries-and-margin fit; guarded isolated non-Preview and natural production evidence that the post-deferral envelope plus reserve stays within the supported allowance/reset; supported reserve; no added provider calls, no deferred/dropped current work, both leagues, and unchanged cron definitions. Ordinary Preview verifies fallback/UI and G4 only. Modeled-only or insufficient production evidence leaves OPS-001 open.
- Rollback/recovery: Revert A3B admission/alert code and non-secret thresholds through review while retaining A3A observation and the attested allowance. If the full scheduled envelope does not fit without admission, use the tested fail-closed safe stop or previous supported policy to keep discretionary future work paused before rollback; never restore unbounded future admission, silently admit future work ahead of current work, or exceed proved capacity.
- User decisions/authority: Approve the evidence-sufficiency conclusion, current-live versus discretionary classification, derived reserve, maximum future deferral/backlog/freshness policy, truthful resume behavior, alert and recovery owners, admission policy, and release. The 2,600-request figure is only a temporary conservative Week 1 schedule-derived reserve, not a proven permanent daily threshold; replace it only with measured reset/weight/burn/retry/margin evidence. Unknown necessary facts block A3B rather than becoming assumptions.
...
- Closure owner: Final owner of OPS-001 jointly with A1 and sufficient A3A evidence only after released production behavior demonstrates either sustainable full-envelope capacity or a sustainable post-deferral envelope that preserves current-live work plus retries and margin, keeps admitted discretionary work within the supported allowance/reset, and resumes deferred work truthfully within the approved freshness bound. A2 is required only when current-live work plus retries and margin otherwise does not fit or the user chooses more capacity. A precautionary A2 upgrade, A3A alone, modeled-but-undemonstrated admission, risk acceptance, unsupported current demand, or any Missing proof leaves OPS-001 open. A3B does not close or manufacture missing TANK-002 evidence.
- Individual prompt:

> In a fresh isolated worktree, execute only A3B from docs/audits/2026-09-04-production-systems-audit-remediation.md after stable A3A provides enough measured evidence to prove either that the full scheduled envelope fits or that unavoidable current-live work plus its retry/cold-isolate envelope and incident margin fits and a reviewed current-before-future policy can keep the post-deferral envelope within the same allowance/reset period. The full pre-admission schedule may exceed the allowance. If a required fact is unknown, current-live work plus retries and margin does not fit, or no truthful bounded deferral policy fits, retain Missing proof and stop; require a proved sustainable A2 change or a separately approved workload/cadence plan when current itself does not fit. Derive the reserve and admitted envelope from measured reset, endpoint billing, natural burn, retry/cold-isolate, demand, and incident-margin evidence. Never defer or degrade current-live work; durably defer and later resume only approved discretionary future work, with zero new provider calls or traffic. Run malformed/conflicting telemetry, both capacity paths and boundaries, reset, hard-cap, retry, grouping, both-league, call-count, backlog/resume, rollback-safe-stop, and G4 gates. Do not change provider plan/key/cron/cadence/scoring/database, merge, or deploy without authorization, and do not claim OPS-001 closed unless the selected capacity/admission path is genuinely demonstrated in production.
...
### B2 — Preview persistence intentionally disabled

- Findings: PREVIEW-001 — Missing proof / Medium / High — Preview/production database target separation is not proved.
- Selected valid outcome: Preview persistence intentionally disabled. No dedicated Preview database, target fingerprint, or Neon integration is required. Preview compact/full snapshot APIs returning the existing unavailable response with HTTP 503 is expected; pages retain their official Sleeper fallback.
- Supported Preview path: B2 closure applies only to Preview deployments built from the released B2 tree. Deployments created before the released B2 code remain untrusted and unsupported for authenticated worker or database testing; this unit does not delete historical Preview deployments.
- Evidence/reproduction: Reproduce the missing Preview exclusion on untouched main at the existing database construction boundary. Separate secret records do not prove different targets; disabling Preview database access removes that target ambiguity without retrieving a credential.
- Dependencies/prerequisites: Retain B1's completed target-identity and catalog evidence, subject to applicable fresh revalidation; its residual application-reader proof does not block B2. G0, G1, G2, G3, G5; explicit Vercel authority to remove exactly the Preview-only DATABASE_URL record. No Neon configuration authority is needed for this outcome.
- Protected behavior: Preview cannot construct a usable database client or access any database, even if DATABASE_URL is accidentally present. Production and local/development database behavior, the existing disabled store, Sleeper fallback, both-league isolation, and every isolated integration safety guard remain unchanged.
- In scope: Return the existing disabled database when VERCEL_ENV=preview at the existing database construction boundary, before URL parsing or Neon client construction. Remove only the separately authorized Preview-only DATABASE_URL record after confirming that it cannot affect Production, Development, or a shared scope, without viewing or preserving its value.
- Excluded: Credential or URL retrieval, copying, fingerprinting, comparison, or logging; new database/client/persistence paths or configuration frameworks; Neon resources or integration changes; production data copy; database connections or SQL; credentialed integration tests in B2; destructive Preview work; other variable changes; migration; permission change; or unrelated app feature change.
- Tests/evidence: Controlled fake fixtures prove Preview stays disabled with absent, valid, malformed, and Production-shaped URLs before parsing/client construction; no database query/write and no projection-worker or Tank01 call follows the disabled persistence path. Ordinary page-level Sleeper fallback remains expected. Production and local/development fixtures preserve existing behavior; both leagues use official Sleeper page fallback; compact/full snapshot APIs return unavailable/503; isolated integration safety remains intact; diagnostics and artifacts contain no credentials or URLs.
- Release checks: G0, G1, G2, G3, G4, G5; exact PR-head Preview created after the Preview-only variable removal; both leagues, fallback, unavailable compact/full APIs, My Team persistence, league switching, and manager-route isolation. Do not force an authenticated Preview worker. Revalidate the current Production SHA under G0; verify exact released SHA and Production compatibility after any separately authorized production release.
- Rollback/recovery: Keep the Preview DATABASE_URL record absent; never restore its old ambiguous value or point Preview at Production. Correct or revert code through a protected PR with Preview still database-disabled. If a separately authorized production release regresses, restore the previous known-good Production deployment and follow with a protected revert. No database rollback is required because B2 changes no schema or business data.
- User decisions/authority: Explicit removal of the one Preview-only DATABASE_URL record and automatic PR Preview deployment; merge and Production release require separate authorization. Stop if removal affects another environment or shared record.
- Change matrix: Code Yes; Configuration Yes, removal of exactly the Preview-only DATABASE_URL record; Billing No; Data No; Production No configuration change, code release only after separate authorization.
- Closure owner: Final owner of PREVIEW-001 and prerequisite for B3 and CI2.
- Individual prompt:

> In a fresh isolated worktree from freshly verified main, execute only B2 from docs/audits/2026-09-04-production-systems-audit-remediation.md with Preview persistence intentionally disabled; do not wait for B1's residual application-reader proof. Return the existing disabled database for VERCEL_ENV=preview before URL parsing or Neon construction, even when DATABASE_URL is accidentally present. Preserve Production/local behavior, the canonical disabled store, official Sleeper fallback, and both-league isolation. With exact user authority, remove only the confirmed Preview-only DATABASE_URL record without viewing or preserving its value; stop on shared or other-environment scope. Prove the boundary with fake fixtures and inspect the exact PR-head Preview after removal, including G4 and expected compact/full API unavailable/503 responses. Run full verify, browser tests, and G5; obtain independent exact-head review. Do not connect to a database, create a Neon resource, change another variable or integration, force an authenticated Preview worker, or begin B3. Open one narrow PR with auto-merge disabled and leave it unmerged; Production release requires separate authorization.
...
- Tests/evidence: Positive normal current/future/observer/read/retention paths; denied immutable/stable-field updates and protected deletes; cross-league attack fixture; source-set/skew/generation/lease concurrency; desired ownership/security-definer/search-path assertions; normalized ACL diff proving only the named new-function ACLs changed. Database testing uses the existing guarded isolated Neon integration harness in apps/site/integration/README.md; ordinary Preview remains persistence-disabled and is not the database test target.
...
- Evidence/reproduction: Reproduce prior success at 12:04:57, a warm one-hour cache, and a 13:00 due attempt returning an observation older than last_succeeded_at. Before selecting a repair, compare the full success/failure/backoff/retry cycle by provider reset period, endpoint class, shared league/period group, and current-versus-future class; uncertainty is not zero delta.
- Dependencies/prerequisites: A3A complete so request impact is measurable; G0, G1, G3, G5. The cache-expiry scheduling path may proceed without A3B only after proving equality of provider attempts, endpoint classes, billed units or a same-endpoint defensible bound, and per-reset admitted demand across the complete cycle. A fresh-ingest path, reset-period shift, any increase in calls/billed units/admitted demand, or any unproved delta requires completed A3B.
- Protected behavior: Database new-observation freshness remains strict; shared cache namespace/TTL and one-action-per-invocation remain unless a deliberate reviewed migration is selected; the zero-delta path adds no provider traffic, and any applicable A3B admission/current-live reserve remains authoritative; no cadence broadening, scoring change, or extra per-league calls.
- In scope: Prefer scheduling after cache expiry when the complete sequence proves zero provider-request and billed-unit delta; otherwise, only after A3B, use an explicit fresh-ingest path whose measured effect fits its supported reserve/admitted envelope. Preserve failure/backoff and later materialization.
- Excluded: Calling a count- or billed-unit-increasing path zero-delta; bypassing A3B for fresh ingest or any admitted-envelope increase; relaxing database freshness; disabling cache globally; cron changes; quota-plan changes; scoring; or future-worker redesign.
- Tests/evidence: Exact warm-cache/prior-success reproduction; no stale success; first failure/backoff and recovery; for the no-A3B path, before/after equality of provider attempts, endpoint classes, billed units/bounds, and admitted demand for normal success, stale-hit failure, backoff, retry/recovery, cold/warm caches, multiple isolates, shared two-league periods, and reset-boundary placement; no shifted request crosses into or increases a different reset-period envelope. Any difference or unknown is an increasing path. For fresh ingest, prove the delta is blocked without A3B and remains inside its admitted envelope without consuming current-live/incident reserve.
- Release checks: G0, G1, G3, G4, G5; exact preview/production SHA; A3A natural reconciliation; for the zero-delta path, full-cycle/per-reset provider-attempt, billed-unit/bound, and admitted-demand equality with both-league grouping; otherwise completed A3B with D2's measured delta inside its supported reserve and post-deferral envelope; both leagues, natural future results, and unchanged cron. No equality proof means no release before A3B.
- Rollback/recovery: Reviewed code revert; never relax the database freshness invariant as rollback.
- User decisions/authority: Normal PR/release authority for a proved zero-delta retiming. Explicitly approve the measured request/admitted-envelope effect for fresh ingest or any increasing/uncertain path.
...
- Closure owner: Final owner of FUTURE-001 only after the zero-delta branch proves its complete-cycle release gate or the increasing/fresh branch passes completed A3B with its delta included.
- Individual prompt:

> In a fresh isolated worktree, execute only D2 from docs/audits/2026-09-04-production-systems-audit-remediation.md after A3A. Reproduce the warm-cache/prior-success failure and first evaluate cache-expiry retiming. It may proceed without A3B only if provider attempts, endpoint classes, billed units/bounds, and admitted demand are equal by reset period, shared league/period group, and current/future class across the complete success/failure/backoff/retry cycle. Treat any uncertainty, increase, or reset-period shift as increasing work. For fresh ingest or an increasing path, stop until A3B is complete, then prove the delta fits its admitted envelope without consuming current-live/incident reserve. Never weaken database freshness, broaden cron/cadence, multiply per-league calls, or change scoring. Open one narrow PR and do not merge or deploy without authorization.
...
- In scope: Observation-only timers/counters on existing canonical paths for worker stages, A3A provider attempts/billed units, database query count/rows/bytes, cleanup, reader waits, and end-to-end lineage; correlation identifiers without league/member/player identity; and a checked-in deterministic load/fault tool that requires an explicit non-production base URL and isolated database fingerprint. Ordinary Vercel Preview is database-disabled and tests fallback/UI behavior only. Database-backed load and fault testing must use the existing guarded isolated Neon test system in apps/site/integration/README.md or another separately authorized non-Preview isolated target.
...
> In a fresh isolated worktree, execute only E0 from docs/audits/2026-09-04-production-systems-audit-remediation.md after A3A and B2 are released and stable. Reuse A3A quota fields and correlation on the canonical provider path; do not create duplicate quota instrumentation. Reproduce the remaining measurement gaps, add only missing bounded non-private instrumentation to existing worker/reader paths, and add a load/fault tool that refuses production and ambiguous targets. Ordinary Vercel Preview is database-disabled and tests fallback/UI behavior only; use the existing guarded isolated Neon test system or another separately authorized non-Preview isolated target for database-backed load and fault testing. Prove timing, query, request, scoring, cache, lease, and response behavior are unchanged. Run G4 and secret-safe gates, open one instrumentation-only PR, and do not tune timeouts, add provider calls/vendors/schema, load-test production, merge, or deploy without authorization.
...
- In scope: Define SLOs and sampling windows before collection; capture p50/p75/p95/p99, maximum, errors/timeouts, cold/warm state, provider attempts/billed units, database query count/rows/bytes, CPU/memory, throttle, and stage/end-to-end duration. Exercise 300 steady visible viewers and a synchronized minute boundary against the authorized non-Preview isolated database-backed target. Treat a percentile as Unverified when the comparable sample size is insufficient; never present a few healthy samples as p99.
...
- Release checks: G0, G2, G4, G5. Verify state/isolation on the authorized isolated target after load; exercise G4 and fallback/UI behavior on ordinary database-disabled Preview. Observe production naturally and read-only; no repository merge or production release occurs in E1.
...
- User decisions/authority: Approve SLOs, sample method, and the non-Preview isolated target/load envelope. Any unexpected cost stops E1 and requires a new approval. If evidence is not representative, E2, E3, and E4 remain blocked.
...
> In a fresh task, execute only E1 from docs/audits/2026-09-04-production-systems-audit-remediation.md after E0 is released and stable. Predeclare the SLOs, sample size, and method, then use the E0 tool to collect representative worker/reader p50/p75/p95/p99 and 300-viewer steady/synchronized evidence against the existing guarded isolated Neon test system or another separately authorized non-Preview isolated target, plus natural read-only production observations. Ordinary Vercel Preview is database-disabled and tests fallback/UI behavior and G4 only; do not use it for database-backed load or fault testing. Do not edit code/config, load-test production, force cron/provider calls, tune timeouts, incur unexpected cost, or expose private payloads. If evidence is insufficient, retain PERF-001 as Missing proof and keep E2–E4 blocked; do not make an OPS-001 closure claim.
...
- Tests/evidence: Every guard negative; owner/runtime URLs with distinct roles are accepted when direct/pooled forms normalize to the same dedicated database; any CI target whose normalized endpoint/database identity matches the production target or denylist is rejected; persistence path runs the full isolated suite; non-persistence path reports not applicable with the same stable context; failed integration fails the context; credentials and row data are suppressed.
...
- Evidence/reproduction: Observe natural 2026 game windows only after A3A makes quota use visible from normal existing calls. Distinguish sanitized aggregate current-live burn, retry/cold-isolate overhead, and discretionary-future burn so the record can support A3B's envelope without guessing. Do not simulate production writes, add provider traffic, or burn quota for evidence. I1 may supply representative natural-burn evidence to A3B but does not need A3B to start.
...
- In scope: Natural pregame → Q1 → quarter → halftime → second half → final; postponed/delayed/OT when naturally available; near-kickoff and future lineup changes; missing projection, empty slot, bye, D/ST, full-precision team sums; source skew; request units/cache/grouping classified as current-live, retry/cold-isolate, or discretionary-future aggregates; worker and browser lineage/adoption; representative tail durations.
...
- Tests/evidence: Exact source/deployment SHA; authority and pre-state revision; non-secret provider remaining/reset; aggregate current-live versus discretionary-future units plus retry/cold-isolate overhead without private payloads; monotonic phase/clock; no publication on invalid input; observation → pending → materialization → acknowledgment; final official convergence; compact/full fence; G4; p50/p75/p95/p99 only with adequate samples.
...
> In a fresh task, execute only I1 from docs/audits/2026-09-04-production-systems-audit-remediation.md after A3A is released and stable and during a natural 2026 game window. Observe and correlate the full transition, lineup, missing/bye/empty/DST/team-total, quota, duration, lineage, browser, both-league, and G4 matrix without forcing cron/provider calls or writing production. Report only sanitized aggregates and exact SHAs, distinguishing current-live, retry/cold-isolate, and discretionary-future burn. Natural-burn evidence may inform A3B, but absent demonstrated A3B reserve and post-deferral admission never closes OPS-001. Do not tune or fix anything; retain every unobserved case as Missing proof and file new defects separately.
...
- Protected behavior: Production variables and values remain untouched; Preview build, cron authentication metadata where applicable, intentionally disabled persistence with DATABASE_URL absent, official Sleeper fallback, and both-league behavior remain. Do not invoke a Preview worker for verification.
...
- Tests/evidence: Before/after name/scope metadata; exact Preview SHA; successful build; Preview persistence disabled and DATABASE_URL still absent; compact/full snapshot APIs return the expected unavailable/503 response; relevant auth metadata; official Sleeper fallback, both leagues, and full G4 pass; G5 artifact scan; approved observation window with no unexpected missing-variable error.
- Release checks: G0, G3, G4, G5 on the exact Preview SHA; no production release.
- Rollback/recovery: Restore only that proven-required Preview variable from the secure owner at minimum Preview scope, never from logs or this plan; never restore Preview DATABASE_URL. Repeat G4 and artifact checks with Preview persistence disabled.
...
> In a fresh task, execute only J1B from docs/audits/2026-09-04-production-systems-audit-remediation.md after J1A proves the exact Preview candidate unused and the user authorizes its removal. Remove only that name from Preview, never view/copy its value, and verify the exact Preview SHA, disabled persistence with DATABASE_URL still absent, expected compact/full snapshot unavailable/503 responses, build, auth metadata, official Sleeper fallback, both leagues, G4, and G5 through the approved observation window. Do not invoke a Preview worker, edit code, touch Production, rotate/revoke anything, or bulk-clean variables. Restore only from the secure owner if a real consumer appears; never restore Preview DATABASE_URL.
...
- Dependencies/prerequisites: J1A and J1B complete and stable; G0, G3, G4, G5; explicit authorization for the named Production variable removal and a new post-removal Production deployment; secure credential owner; separate explicit rotation/revocation authorization when applicable.
- Protected behavior: All other Production variables, minimum scopes, cron authentication, provider/database access, fallback, exact approved source Git SHA, and both leagues remain unchanged. The required post-removal deployment has a new deployment ID; it does not imply a source change.
- In scope: Remove only the approved unused name from Production, then create a new Production deployment after removal from the exact approved Git SHA and make that new Ready deployment the Production alias target so the serving runtime cannot retain the earlier build environment. The same approved Git SHA may be redeployed. If the secure owner confirms the name represents a live credential, execute only the separately approved rotation or revocation and update only already-approved consumers through their secure managers; record names/scopes/status, never values.
- Excluded: Source change or merge merely to force a build; value viewing/copying/comparison; bulk environment cleanup; unrelated credential rotation; required-variable rename; provider plan/database change; reusing a deployment created before removal as closure evidence; or any Production deployment beyond the one approved post-removal deployment and an authorized recovery deployment.
- Tests/evidence: Pre-removal Ready deployment ID/time/SHA; before/after name/scope metadata and secure-owner disposition; removal timestamp; a new Ready Production deployment created after that timestamp; its immutable deployment ID, creation time, exact approved Git SHA, source repo/branch/root binding, and proof that the Production alias serves it; successful build; natural cron-auth result; both leagues and full G4; G5 artifacts; approved observation window. A prior deployment or matching SHA alone is not post-removal proof.
- Release checks: G0, G3, G4, G5; the exact new post-removal Ready deployment ID/time/source/SHA and Production-alias target; both leagues; no unrelated Vercel, provider, database, or schedule setting changes. J1C cannot close if that deployment is absent, failed, not serving Production, or not created after removal.
- Rollback/recovery: If the variable is proven required, restore only the named item from its secure owner at minimum scope, create a new recovery deployment after restoration from the approved Git SHA, make that Ready deployment the Production alias target, and repeat release checks. Do not recover by re-aliasing a pre-removal deployment that lacks the restored environment. A revoked credential is replaced through the secure issuer, never recovered from logs or this plan.
- User decisions/authority: Treat named Production variable removal, one post-removal Production redeploy of the exact approved SHA, observation, and any credential rotation/revocation as separate grants; none authorizes another. Name each applicable action and owner. No grant permits viewing or copying a value.
- Change matrix: Code No; Configuration Yes, the named Production variable and separately approved credential disposition only; Billing No; Data No; Production Yes, named configuration/credential state plus one explicitly authorized post-removal deployment of the exact approved SHA.
- Closure owner: Final owner of ENV-001 only after consumer proof, Preview observation, authorized Production retirement and credential disposition, and a new Ready Production deployment created after removal passes every production check.
- Individual prompt:

> In a fresh task, execute only J1C from docs/audits/2026-09-04-production-systems-audit-remediation.md after stable J1A/J1B evidence and separate exact user authorizations for removal and redeployment. Reconfirm the named Production variable is unused, record the removal time, remove only it, then create one new Ready Production deployment after removal from the exact approved Git SHA and prove the Production alias serves it. An older deployment or matching SHA alone cannot close ENV-001. Rotate/revoke an underlying live credential only when that separate exact action is authorized by its secure owner. Never reveal/copy/compare values, bulk-clean, change source, or merge merely to trigger a build. Record the immutable deployment ID/time/SHA/source/branch/root/alias and verify natural auth, both leagues, G4, and G5; do not touch unrelated Vercel/provider/database/schedule state.
...
| LEDGER-001 | I2 only. |
| ENV-001 | J1A inventory + J1B Preview proof + J1C separately authorized Production retirement/disposition + a new post-removal Ready Production deployment with exact ID/time/SHA/source/alias verification. |
