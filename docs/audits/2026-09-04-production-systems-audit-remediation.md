...
- A minimal unchanged-application read against a verified restored target must pass for League One and League Two no later than B4's pre-cutover gate. B4 production permission revocation and any production deletion-capable unit remain blocked until that proof passes. Independent feature work that does not change database permissions or delete production data is unblocked by this residual item, subject to its own explicit prerequisites.
...
| 5 | B1 Recovery and schema proof | DB-PROOF-001 | G0; isolated restore authority | Catalog/restore evidence complete; application-reader proof remains open |
...
| 8 | B4 Runtime privilege cutover | DB-001 | B3 released and stable; fresh restore point; both-league restored-target application read passed before cutover | Exact grants only |
...
- In scope: Retain the completed catalog, restore, integrity, capacity/maintenance, and G6 selective-recovery evidence above; complete only the minimal restored-target application read for both leagues in a separately authorized task, with current identity and before/after integrity checks.
...
- Tests/evidence: Retain the completed evidence; the remaining pass must invoke the unchanged `readStoredMatchupRevision` through the application for `league1` and `league2` against a verified isolated restored target, with the expected runtime identity, enforced read-only operation, recovery-time-appropriate freshness evaluation, successful selection/TypeScript decoding, league isolation, and unchanged before/after integrity aggregates. SQL-only reads and offline harness tests do not supply that pass. DB-PROOF-001 remains open until this minimal application proof is retained, no later than B4's pre-cutover gate.
...
- Closure owner: B1 owns the narrow residual DB-PROOF-001 proof. B2 and B3 use its completed evidence under G2; the remaining application read is mandatory before B4 production permission revocation or any production deletion-capable unit, not a blanket block on independent feature work.
- Individual prompt:

> In a fresh separately authorized task, complete only B1's residual proof from docs/audits/2026-09-04-production-systems-audit-remediation.md. Retain the successful historical catalog/restore/recovery evidence and its limitations; do not repeat the audit. Revalidate identity and run the minimal unchanged restored-target application read for League One and League Two with enforced read-only operation, recovery-time-appropriate freshness, secret safety, production exclusion, and before/after integrity checks. Retain a separate proof supplement and leave the historical B1 report unchanged. Require exact-ID cleanup, verified removal, and explicit destructive-action confirmation. Stop on missing authority, ambiguous target, unexpected cost, or a failed read. This prompt does not authorize Task B or another attempt; keep DB-PROOF-001 open until the application proof passes, and block B4 production revocation and production deletion-capable units until then.
...
- Tests/evidence: Retained passing B1 residual application-read proof for both leagues before cutover; B3 positive/negative suite under the final role; complete catalog/grant diff; compromised-runtime transaction cannot mutate protected history or cross league scope; natural current/future/observer/public reads remain healthy.
...
> In a fresh isolated worktree, execute only B4 from docs/audits/2026-09-04-production-systems-audit-remediation.md after B3 is deployed and proven. Require passing minimal unchanged restored-target application reads for League One and League Two no later than the pre-cutover gate; production permission revocation remains blocked until that proof passes. Reproduce the excess grants, apply the reviewed exact grant manifest, and verify every normal path plus all denied mutations under the final runtime role. Do not change application features, scoring, retention, cron, provider settings, or business data. Require a fresh restore point and pre-reviewed narrow break-glass grants; open one privilege-only PR and do not merge, migrate, or deploy without explicit database and release authorization.
...
| DB-PROOF-001 | B1 retains the completed catalog, timed restore, integrity, capacity/maintenance inspection, and synthetic selective-recovery evidence. Open only for minimal unchanged restored-target application reads for both leagues, required no later than B4's pre-cutover gate; no full closure is claimed. Task A tooling requirements are not universal closure gates. |
| PREVIEW-001 | B2 only. |
| DB-001 | B3 may use completed B1 catalog/restore evidence plus B2 and its own isolated testing. B4 closes only after the residual both-league restored-target application read passes before cutover and exact-grant production verification succeeds. DB-001 remains open. |
...
| RET-001, RET-002 | F0 design may use completed B1 evidence; F1 narrow database interface and F2 owner/enablement retain B4 and approved horizons/recovery gates. Production deletion-capable units remain blocked until the residual both-league restored-target application read passes. |
