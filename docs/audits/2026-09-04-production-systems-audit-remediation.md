...
- Dependencies/prerequisites: A3B complete so request impact is measurable inside its supported reserve and admitted envelope; G0, G1, G3, G5.
- Protected behavior: Database new-observation freshness remains strict; shared cache namespace/TTL and one-action-per-invocation remain unless a deliberate reviewed migration is selected; D2 never bypasses A3B admission or consumes the current-live reserve; no cadence broadening, scoring change, or extra per-league calls.
...
- Tests/evidence: Exact warm-cache/prior-success reproduction; no stale success; first failure/backoff and recovery; before/after provider request counts recalculated inside A3B's admitted envelope; proof that any fresh-ingest delta cannot bypass admission or consume current-live/incident reserve; cold/warm and same-period sharing.
- Release checks: G0, G1, G3, G4, G5; exact preview/production SHA; supported A3B reserve and post-deferral admitted envelope including D2's measured request delta; both leagues; natural future results and unchanged cron.
...
> In a fresh isolated worktree, execute only D2 from docs/audits/2026-09-04-production-systems-audit-remediation.md after A3B. Reproduce the warm-cache/prior-success failure, then align scheduling to expiry plus supported headroom or implement a deliberate fresh-ingest path with measured request counts. Recalculate any request delta inside A3B's admitted envelope; never bypass admission or consume the current-live/incident reserve. Never weaken database freshness, broaden cron/cadence, multiply per-league calls, or change scoring. Open one narrow PR and do not merge or deploy without authorization.
