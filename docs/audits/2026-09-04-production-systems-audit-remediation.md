...
- Release checks: G0, G1, G3, G4, G5; exact preview and production SHA; A3A natural-call reconciliation; proved full-demand fit or proved current-live-plus-retries-and-margin fit; Preview/isolated and natural production evidence that the post-deferral envelope plus reserve stays within the supported allowance/reset; supported reserve; no added provider calls, no deferred/dropped current work, both leagues, and unchanged cron definitions. Modeled-only or insufficient production evidence leaves OPS-001 open.
...
- Tests/evidence: Controlled fake fixtures prove Preview stays disabled with absent, valid, malformed, and Production-shaped URLs before parsing/client construction; no database query/write or worker/provider work follows the disabled persistence path. Production and local/development fixtures preserve existing behavior; both leagues use official Sleeper page fallback; compact/full snapshot APIs return unavailable/503; isolated integration safety remains intact; diagnostics and artifacts contain no credentials or URLs.
