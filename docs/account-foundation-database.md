...
Migration `021_website_auth.sql` separately adds Better Auth users, credentials, sessions, verification records and persisted rate limits in `website_auth`. The application runs maintained Better Auth; Neon stores both schemas. The account-domain role and the auth role have reciprocal access restrictions. See [the authentication boundary](account-auth-foundation.md) for runtime configuration and reset/session guarantees.
...
Inspect the existing roles and their consumers before provisioning. After migration 020, `apps/site/scripts/provision-account-role.sql` runs as the schema owner and creates `league_one_account` only if absent. It checks an existing role, refuses elevated flags, incoming memberships or owned objects, resets both table and column ACLs, then grants only the intended privileges. It verifies RLS, worker exclusion, source-write denial, restricted lifecycle columns and unavailable privileged source functions. No password appears in this script. Coordinate any authorized password assignment with existing consumers and rerun the postconditions after any service control-plane role operation.

After migration 021, `apps/site/scripts/provision-auth-role.sql` creates or checks the separate restricted `league_one_auth` role and grants the required `website_auth` table access. It denies auth access to public account/league data and denies account/worker access to auth tables. Runtime credentials must use their respective restricted roles, never the owner. Applying a migration or provisioning a role does not itself authorize account activation.
...
Targets including migration 021 also provision and check the separate auth role. The destructive harness resets `public` and `website_auth` only in a fresh disposable database after all existing guards pass. It must never target production or the retained populated pilot. Managed `neon_auth` storage is not a reset target. Actual auth lifecycle proofs additionally require the explicitly supplied restricted auth-role connection; see [the isolated harness instructions](../apps/site/integration/README.md).
...
Migrations 001–019 remain unchanged. Inspect installed checksums and apply only actually pending reviewed migrations; 020 and 021 are not instructions to replay an installed migration. A failed migration transaction rolls back. After a committed deployment, disable the account feature or use a compatible application rollback/forward fix; do not erase private data, auth records, identity/audit history or migration-ledger entries as routine rollback. Preserve the pre-account recovery checkpoint separately from ongoing account/auth backups and their required service configuration.
