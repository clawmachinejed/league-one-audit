...
The account UI associates one of the public Sleeper profiles already observed in the supported leagues. Before saving, it retrieves that profile's current NFL-season league list and a bounded set of matching roster examples. The user confirms the profile and then one team; a wrong-team choice can show another matching example. This is recognition evidence only. The server accepts previews solely for currently selectable observed profiles, rechecks the session and availability after provider work, and keeps the stored assurance `user_asserted`. It does not request Sleeper credentials, search arbitrary users, claim exclusive ownership or automatically treat `eneerg` display corrections as identity proof. Multiple website accounts may associate the same public profile. They still have separate private preferences. If current team evidence is unavailable, the user can retry later rather than being shown an invented team.
