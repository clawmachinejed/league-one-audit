...
Registration reads preserve every intended member as either ready or explicitly unavailable. Current and future lineup workers can progress ready leagues while reporting unavailable members; failed registrations retain their existing watches and their place in maintenance rotation. Public source resolution and a targeted administration operation query only the requested league. An all-league administration operation reports unavailable registrations alongside healthy results. Empty enrollment and database failures remain errors, and enabled persistence never falls back to bootstrap IDs. The strict enrollment reader remains available for operations requiring complete intended membership; unavailable registrations cannot silently shrink a publication group.
