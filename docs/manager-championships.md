...
The asset is RGBA1145×1373, with real transparent pixels. Next Image serves a size-optimized local image. CSS height is 1ex, the manager font lowercase x-height, with the original1145:1373 aspect ratio. Repeated icons share the same image URL. The icon group has one accessible label containing the number of League One championships and each year; individual decorative images have empty alt text. Trophies inherit the name font size and sit on its baseline. Names and icons may wrap naturally; no count replacement, font shrinking or new truncation is introduced. Existing enclosing button/link labels also include title counts and years for assistive technology.
