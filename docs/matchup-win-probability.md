...
The expected finishing margin is the difference between the two existing canonical projected team totals, including an evidenced official team adjustment (official team total minus the sum of supplied starter scores). An adjustment is used only when all those scores are finite. Existing displayed scores, `clock-v1` totals, standings and results remain unchanged.
...
- Missing starters, incomplete baselines, untrusted game context, missing required actuals or retained-prior projections make only that matchup's estimate unavailable. They never turn missing evidence into a confident zero.
...
Both current and future materialization use the shared builder. `normal-v1` is included in publication revision material so an old revision cannot conflict with the new payload. Existing content hashing includes probabilities and the existing retention/deduplication behavior remains in effect. The same unchanged inputs produce identical probabilities without a new timestamp or random simulation.
...
Release needs the repository verification workflow, independent review and the actual Vercel preview. No migration or data operation is needed. After release, verify the exact merged SHA, all three league readers and the first ordinary current-week snapshot carrying `normal-v1`; check both pages and an exact future week after its next materialization. A preview without production snapshots cannot demonstrate live probability availability. Synthetic UI fixtures must be identified as such.

Rollback is a compatible code revert. Old readers tolerate the extra JSON fields; old snapshot rows and official results remain intact. Do not delete or rewrite history. Future coefficient changes require a new probability model version and calibration evidence, not changes to `clock-v1`.
