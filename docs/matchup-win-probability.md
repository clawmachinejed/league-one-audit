...
The expected finishing margin is the difference between the two canonical projected team totals, including an evidenced official team adjustment (official team total minus the sum of supplied starter scores). An adjustment is used only when all those scores are finite. The active-week non-playing designation policy below sets the affected starter's expected remaining contribution to zero. Official scores and results remain unchanged; projected totals and projected standings use the corrected forecast through the existing shared path.

Each unfinished starter expected to contribute points receives a normal error distribution. Its standard deviation is the larger of a fixed floor and the absolute pregame baseline multiplied by a position coefficient:
...
- An occupied starting slot with an explicit current non-playing designation contributes zero expected remaining points and zero remaining variance in the active scoring week. Before kickoff its projection is zero; during a game its forecast retains all official points already scored. A missing provider baseline does not veto that explicit product assumption, and an old positive baseline does not override it. A designation alone does not make a valid probability unavailable. The actual starter stays in the lineup. Bench players remain excluded from odds.
- The source explicitly scopes current injury metadata to the site's active season/week using the existing calendar, not Sleeper's display leg. Past/future requests, preseason, a completed season or unavailable calendar authority do not receive this override. The shared availability classifier recognizes OUT, INACTIVE, SUSPENDED/SUS, IR, PUP and NFI, their full-name aliases and compound flags. Questionable, Doubtful, Probable and unknown text retain ordinary projection inputs; those labels never veto an otherwise valid calculation. Canonical exact-period game context and required actuals remain necessary.
...
- Final players always retain official points and their existing frozen pregame display values, even if they are currently Out or on IR. Immutable baselines are never rewritten. A nonzero actual paired with pregame context is not erased by the non-playing designation policy.
...
- Explicit empty slots are valid and add no variance. An entirely absent lineup remains unavailable. If valid inputs leave zero remaining variance, nonfinal output remains an estimate: equal projected finishes use 50% each, while a lead uses the existing strict-interior probability limits. A collection of inactive starters, byes or empty slots does not claim official finality or hide the estimate.
...
Release needs the repository verification workflow, independent review and the actual Vercel preview. No migration or data operation is needed. After release, verify the exact merged SHA, all three league readers and the first ordinary current-week snapshot carrying `normal-v3`; check the Dynasty matchup with the IR starter, all shared matchup views, and an exact future week after its next materialization. A preview without production snapshots cannot demonstrate live probability availability. The retained public Week 2 fixture contains real starter values but constructed canonical baseline-quality and game-state fields; it is a regression case, not proof of private production baseline contents.

Deploy the updated readers and worker together. A rollback must retain `normal-v3` reader support: an older application that accepts only v1/v2 rejects v3 snapshots. If the forecast policy needs reverting, revert its producer changes while retaining v1/v2/v3 validation and presentation, then let the existing worker publish normally. Do not delete or rewrite history. Official results and frozen baselines remain intact. Future coefficient changes require a new probability model version and calibration evidence, not changes to `clock-v1`.
