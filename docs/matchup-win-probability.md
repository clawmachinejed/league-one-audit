...
The expected finishing margin is the difference between the two canonical projected team totals, including an evidenced official team adjustment (official team total minus the sum of supplied starter scores). An adjustment is used only when all those scores are finite. The active-week Out/IR policy below sets the affected starter's expected remaining contribution to zero. Official scores and results remain unchanged; projected totals and projected standings use the corrected forecast through the existing shared path.
...
- Final players always retain official points and their existing frozen pregame display values, even if they are currently Out or on IR. Immutable baselines are never rewritten. A nonzero actual paired with pregame context is not erased by the Out/IR policy.
